...
folds = 10
nObs=40
idx<-seq(1,nObs,nObs/folds)
idx[length(idx)+1]=nObs

for (i in seq(folds)){
  idx.tf<-rep(FALSE,nObs)
...
####### Cross Validate Function (Ridge)
library(pracma)
cross_validate<-function(Y,X,lambda){
  ...
  idx<-seq(1,nObs,nObs/folds)
  idx[length(idx)+1]=nObs
  # yhat.cv = matrix(0,dim(Y)[1],dim(Y)[2],length(lambda))
  yhat.cv = array(0, dim = c(dim(Y)[1],dim(Y)[2],length(lambda)))
  #print(dim(yhat.cv))
  ...
  for (l in seq(length(lambda))){
    ...
    for (i in seq(folds)){
      ...
      data.cv$X = X[!idx.tf,]
      #print(dim(X))
      Bhat.ridge = solve(t(X)%*%X+diag(dim(X)[2])*ll)%*%t(X)%*%Y
      
      data.cvo = list()
      data.cvo$X<-X[idx.tf,]
      data.cvo$Y<-Y[idx.tf,]
      Yhat = data.cvo$X%*%Bhat.ridge
      #print(dim(Yhat))
      yhat.cv[idx.tf,,l]<-Yhat
    }
  }
  # calculate R2 cross validation
  return(yhat.cv)
}
